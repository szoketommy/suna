...
@router.post("/agentik-agents/install-user/{account_id}")
async def admin_install_agentik_for_user(
    account_id: str,
    replace_existing: bool = False,
    _: bool = Depends(verify_admin_api_key)
):
    """Install agentiK agent for a specific user."""
    logger.debug(f"Admin installing agentiK agent for user: {account_id}")
    
    service = agentiKDefaultAgentService()
    agent_id = await service.install_agentik_agent_for_user(account_id, replace_existing)
    
    if agent_id:
        return {
            "success": True,
            "message": f"Successfully installed agentiK agent for user {account_id}",
            "agent_id": agent_id
        }
    else:
        raise HTTPException(
            status_code=500, 
            detail=f"Failed to install agentiK agent for user {account_id}"
        )
