...
def extract_agent_config(agent_data: Dict[str, Any], version_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Extract agent configuration with simplified logic for agentiK vs custom agents."""
    ...
    if os.getenv("ENV_MODE", "").upper() == "STAGING":
        print(f"[DEBUG] extract_agent_config: Called for agent {agent_id}, is_agentik_default={is_agentik_default}")
    ...
    # Handle agentiK agents with special logic
    if is_agentik_default:
        return _extract_agentik_agent_config(agent_data, version_data)
    
    # Handle custom agents with versioning
    return _extract_custom_agent_config(agent_data, version_data)


def _extract_agentik_agent_config(agent_data: Dict[str, Any], version_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Extract config for agentiK agents - always use central config with user customizations.
    
    Uses cached static config from runtime_cache instead of reading AGENTIK_CONFIG directly.
    Always overrides name from AGENTIK_CONFIG regardless of what's in the database.
    """
    ...
    agent_id = agent_data.get('agent_id', 'Unknown')
    logger.debug(f"Using agentiK central config for agent {agent_id}")
    ...
    # Start with cached static config, but always override name from AGENTIK_CONFIG
    config = {
        'agent_id': agent_data['agent_id'],
        'name': AGENTIK_CONFIG['name'],  # Always override name from AGENTIK_CONFIG
...
def build_unified_config(
    ...
    agentik_metadata: Optional[Dict[str, Any]] = None,
    ...
    if agentik_metadata:
        config['agentik_metadata'] = agentik_metadata
