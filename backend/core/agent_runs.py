...
async def _find_shared_agentik_agent(client):
    ...
    if admin_user_id:
        admin_agentik = await client.table('agents').select('agent_id').eq('account_id', admin_user_id).eq('metadata->>is_agentik_default', 'true').maybe_single().execute()
        
        if admin_agentik and admin_agentik.data:
            loader = await get_agent_loader()
            agent_data = await loader.load_agent(admin_agentik.data['agent_id'], admin_user_id, load_config=True)
            logger.info(f"✅ Using system agentiK agent from admin user: {agent_data.name} ({agent_data.agent_id})")
            return agent_data
        else:
            logger.warning(f"⚠️ SYSTEM_ADMIN_USER_ID configured but no agentiK agent found for user {admin_user_id}")
    
    any_agentik = await client.table('agents').select('agent_id, account_id').eq('metadata->>is_agentik_default', 'true').limit(1).maybe_single().execute()
    
    if any_agentik and any_agentik.data:
        loader = await get_agent_loader()
        agent_data = await loader.load_agent(any_agentik.data['agent_id'], any_agentik.data['account_id'], load_config=True)
        logger.info(f"Using shared agentiK agent: {agent_data.name} ({agent_data.agent_id})")
        return agent_data
    
    logger.error("❌ No agentiK agent found! Set SYSTEM_ADMIN_USER_ID in .env")
    return None
...
async def _load_agent_config(client, agent_id: Optional[str], account_id: str, user_id: str, is_new_thread: bool = False):
    ...
    if agent_id:
        from core.runtime_cache import get_static_agentik_config, get_cached_user_mcps
        ...
        if static_config and cached_mcps is not None:
            ...
            logger.info(f"⚡ [FAST PATH] agentiK config from memory + Redis MCPs: {(time.time() - t_start)*1000:.1f}ms (zero DB calls)")
        else:
            t_loader = time.time()
        ...
        if is_new_thread:
            from core.utils.ensure_agentik import ensure_agentik_installed
            await ensure_agentik_installed(account_id)
        
        default_agent = await client.table('agents').select('agent_id').eq('account_id', account_id).eq('metadata->>is_agentik_default', 'true').maybe_single().execute()
        ...
        if default_agent and default_agent.data:
            ...
            logger.warning(f"[AGENT LOAD] No default agent found for account {account_id}, searching for shared agentiK")
            agent_data = await _find_shared_agentik_agent(client)
