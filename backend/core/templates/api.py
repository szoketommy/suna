...
from .template_service import (
    ...
    agentiKDefaultAgentTemplateError
)
...
class TemplateResponse(BaseModel):
    ...
    categories: List[str]
    is_public: bool
    is_agentik_team: Optional[bool] = False
...
async def create_template_from_agent(
    ...
    except agentiKDefaultAgentTemplateError as e:
        logger.warning(f"Template creation failed - agentiK default agent: {e}")
...
@router.get("/agentik-all", response_model=MarketplaceTemplatesResponse)
async def get_all_agentik_templates(
    ...
    try:
        ...
        try:
            ...
            error_str = f"Error of type {type(e).__name__}"
        logger.error(f"Error getting all agentiK templates: {error_str}")
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def get_marketplace_templates(
    ...
    is_agentik_team: Optional[bool] = Query(None, description="Filter for agentiK team templates"),
    ...
    try:
        ...
        filters = MarketplaceFilters(
            search=search,
            tags=tags_list,
            is_agentik_team=is_agentik_team,
