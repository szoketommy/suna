...
export function useMarketplaceTemplates(params?: {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<MarketplaceTemplatesResponse> => {
      ...
      if (params?.is_agentik_team !== undefined) searchParams.set('is_agentik_team', params.is_agentik_team.toString());
...
export function useagentiKTeamTemplates(options?: { enabled?: boolean }) {
  return useQuery({
    queryKey: ['secure-mcp', 'agentik-templates-all'],
    queryFn: async (): Promise<MarketplaceTemplatesResponse> => {
      const supabase = createClient();
      const { data: { session } } = await supabase.auth.getSession();

      if (!session) {
        throw new Error('You must be logged in to view agentiK templates');
      }

      const response = await fetch(`${API_URL}/templates/agentik-all`, {
        headers: {
          'Authorization': `Bearer ${session.access_token}`,
        },
      });
