...
interface AgentSelectionState {
  ...
  isagentiKAgent: (agents: Agent[]) => boolean;
}
  ...
  persist(
    (set, get) => ({
      ...
      initializeFromAgents: (agents: Agent[], threadAgentId?: string, onAgentSelect?: (agentId: string | undefined) => void) => {
        ...
        if (!selectedId) {
          ...
          if (current && agents.some(a => a.agent_id === current)) {
            ...
            const defaultagentiKAgent = agents.find(agent => agent.metadata?.is_agentik_default);
            selectedId = defaultagentiKAgent ? defaultagentiKAgent.agent_id : agents[0].agent_id;
          }
        }
      ...
      autoSelectAgent: (agents: Agent[], onAgentSelect?: (agentId: string | undefined) => void, currentSelectedAgentId?: string) => {
        ...
        const defaultagentiKAgent = agents.find(agent => agent.metadata?.is_agentik_default);
        const agentToSelect = defaultagentiKAgent || agents[0];
      ...
      isagentiKAgent: (agents: Agent[]) => {
        const { selectedAgentId } = get();
        const currentAgent = selectedAgentId 
          ? agents.find(agent => agent.agent_id === selectedAgentId)
          : null;
        return currentAgent?.metadata?.is_agentik_default || selectedAgentId === undefined;
      },
    }),
    {
...
export const useIsagentiKAgentFn = () => 
  useAgentSelectionStore((state) => state.isagentiKAgent);
...
export const useAgentSelection = () => {
  ...
  return {
    ...
    isagentiKAgent: store.isagentiKAgent,
  };
}; 