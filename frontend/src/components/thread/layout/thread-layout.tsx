...
import { agentiKComputer, ToolCallInput } from '@/components/thread/agentik-computer';
...
export const ThreadLayout = memo(function ThreadLayout({
  ...
  // agentiK Computer Store - for handling file open requests
  const { shouldOpenPanel, clearShouldOpenPanel, openFileInComputer, openFileBrowser } = useagentiKComputerStore();
  ...
  // Handle file click - now opens in agentiK Computer instead of modal
  ...
  const selectedFilePath = useagentiKComputerStore((state) => state.selectedFilePath);
