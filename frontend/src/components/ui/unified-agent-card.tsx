...
const CardAvatar: React.FC<{ 
  ...
  const isagentiKAgent = data.metadata?.is_agentik_default === true;
...
const MarketplaceBadge: React.FC<{ 
  isagentiKTeam?: boolean; 
  isOwner?: boolean;
}> = ({ isagentiKTeam, isOwner }) => (
...
const AgentBadges: React.FC<{ data: BaseAgentData, isagentiKAgent: boolean }> = ({ data, isagentiKAgent }) => (
  <div className="flex gap-1">
    {!isagentiKAgent && data.current_version && (
      <Badge variant="outline" className="text-xs">
        <GitBranch className="h-3 w-3 mr-1" />
        {data.current_version.version_name}
      </Badge>
    )}
    {!isagentiKAgent && data.is_public && (
...
export const UnifiedAgentCard: React.FC<UnifiedAgentCardProps> = ({
  ...
  const renderStandardCard = () => {
    ...
    const renderBadge = () => {
      switch (variant) {
        case 'marketplace':
          return <MarketplaceBadge isagentiKTeam={data.is_agentik_team} isOwner={isOwner} />;
        case 'template':
          return <TemplateBadge isPublic={data.is_public} />;
        case 'agent':
          return <AgentBadges data={data} isagentiKAgent={isagentiKAgent} />;
        default:
          return null;
      }
    };
